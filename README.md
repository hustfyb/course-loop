...
面向《软件工程 3.0》的文档驱动课程维护、邮箱验证码登录（首次登录后补全身份信息）、Team 协作与实验评分网站。管理员维护课程并与 Pi 对话完善草案、确认发布；教师基于已发布课程开设独立课堂；学生凭课堂邀请码加入课堂后按 Team 提交作业（每实验最多 3 次，取最高分）。Agent 按 rubric 输出有证据的分项评价，网站校验及计算成绩。内置全站使用助手「小课」（pi print 单发模式 + `--no-tools`，历史随 prompt 携带，Pi 不可用时诚实降级）与意见收集/管理（用户提改进意见，管理员在「意见管理」查看并标记处理）。
...
正式快照不可修改。发布后的课程修改进入新草案。每个实验由课程内容设定判分方式：小组判分（组长代表小组提交）或个人判分（学生本人提交，无需小组）。**统一提交模型**：每实验最多提交 3 次（小组实验按组计、个人实验按人计），每次提交立即评分并发布，取最高分作为该实验成绩；报告保留所有版本，教师保留复核修改成绩的权利。
