...
const accessHeaders=config.siteAccessToken?{'OAI-Sites-Authorization':'Bearer '+config.siteAccessToken}:{};const base=origin.origin;const root=path.resolve(here,config.workRoot||'work');await fs.mkdir(root,{recursive:true});
...
async function request(endpoint,body,lease){const response=await fetch(base+'/api/connector/'+endpoint,{method:'POST',headers:{...accessHeaders,'Content-Type':'application/json','Authorization':'Bearer '+config.token,...(lease?{'x-job-lease':lease}:{})},body:JSON.stringify(body),signal:AbortSignal.timeout(30000)});const r=await response.json();if(!response.ok)throw Error(r.error||'网站请求失败');return r;}
...
async function processLoop(kind){while(!stopped){try{const {job}=await request('poll',{acp:kind==='acp'&&!!config.acp?.enabled,mail:kind==='mail'&&!!mail});if(!job){await new Promise(r=>setTimeout(r,config.pollIntervalMs||5000));continue;}let beat;try{
 ...
 else {const work=path.join(root,job.id+'-'+job.lease);await fs.mkdir(work,{recursive:true});const docs=[];for(const file of job.files||[]){const r=await fetch(base+`/api/connector/file/${job.id}/${file.id}`,{headers:{...accessHeaders,Authorization:'Bearer '+config.token,'x-job-lease':job.lease},signal:AbortSignal.timeout(30000)});if(!r.ok)throw Error('无法下载任务附件');const bytes=await r.arrayBuffer();if(bytes.byteLength>20*1024*1024)throw Error('附件超过大小限制');const fname=file.name.replace(/[\\/:\x00-\x1f]/g,'_');const dir=path.join(work,file.id);await fs.mkdir(dir,{recursive:true});const filePath=path.join(dir,fname);await fs.writeFile(filePath,Buffer.from(bytes));const content=await extractDocument(fname,bytes,dir);docs.push(`文件 ${file.name}：${filePath}\n${content}`);}
