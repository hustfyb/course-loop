...
export function promptFor(job,docs){const p=job.payload;let contract;
 ...
 else if(job.kind==='grade')contract='返回 {"items":[{"id":"评分项编号","score":数值,"reason":"评分理由，末尾写明这条评分项的具体改进建议","evidence":["文件名/测试结果与具体位置"]}],"baseline":true,"suggestions":["2-4 条针对整个提交的可操作改进建议"],"needsReview":布尔值,"limitations":["不能验证的内容"]}。items 必须与 rubric 逐项对应、数量完全一致，score 不超过各项 max，按评分项如实给分并给真实证据。baseline 表示提交是否与实验要求相关且有实质内容：相关且有实质内容为 true（值得 60 分保底），与要求完全无关或几乎为空为 false；平台会把 baseline=true 的提交总分映射到 60-100 区间（60+0.4×原始总分），所以你只需如实评分，不要自行校准松紧。每个 reason 末尾写该评分项的改进建议，把缺失项写成改进建议。运行项目时使用课程约定测试，不能修改测试以通过。';
